...
export type AugmentedPlanet = WithType<WithId<Planet>, "Planet">;

export const transformPlanet = (planet: Planet): AugmentedPlanet => ({
  ...planet,
  id: extractId(`${SWAPI_ENDPOINT}/planets`, planet.url),
  films: planet.films.map(extractForeignerKey),
  residents: planet.residents.map(extractForeignerKey),
  type: "Planet",
});
