...
import { extractForeignerKey, extractId, WithId, WithType } from "./utils";
...
export type AugmentedPerson = WithType<WithId<Person>, "Person">;

export const transformPerson = (person: Person): AugmentedPerson => ({
  ...person,
  id: extractId(`${SWAPI_ENDPOINT}/people`, person.url),
  url: extractForeignerKey(person.url),
  homeworld: extractForeignerKey(person.homeworld),
  films: person.films.map(extractForeignerKey),
  vehicles: person.vehicles.map(extractForeignerKey),
  starships: person.starships.map(extractForeignerKey),
  type: "Person",
});
